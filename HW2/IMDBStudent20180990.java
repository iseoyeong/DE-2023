...
public class IMDBStudent20180990 {
    ...
    public static class IMDB {
        ...
        public IMDB(String movie, double avgRating) {
            this.movie = movie;
            this.avgRating = avgRating;
        }

        public String getMovie() { return this.movie; }
        public Double getRating() { return this.avgRating; }
    }
    ...
    public static class IMDBComparator implements Comparator<IMDB> {
        public int compare(IMDB x, IMDB y) {
            if (x.avgRating > y.avgRating) return 1;
            if (x.avgRating < y.avgRating) return -1;
            return 0;
        }
    }
    ...
    public static void insertIMDB(PriorityQueue q, String movie, double avgRating, int topK) {
        IMDB imdb_head = (IMDB) q.peek();
        if (q.size() < topK || imdb_head.avgRating < avgRating) {
            IMDB imdb = new IMDB(movie, avgRating);
            q.add(imdb);
    ...
    public static class IMDBMapper extends Mapper<Object, Text, DoubleString, Text> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (movieFile) {
                ...
                String id = movie[1];
                String avgRating = movie[2];

                outputKey = new DoubleString(id, "Ratings");
                outputValue.set("Ratings::" + avgRating);
                context.write( outputKey, outputValue );
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (otherArgs.length != 3) {
            System.err.println("Usage: IMDB20180990 <in> <out> <k>");
            System.exit(2);
        }
